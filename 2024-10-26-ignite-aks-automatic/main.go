...
func main() {
	pulumi.Run(func(ctx *pulumi.Context) error {
		...
		randomName := pulumi.Sprintf("ignited%v", randomPet.ID())
		...
		databaseAccount, err := documentdb.NewDatabaseAccount(ctx, "databaseAccount", &documentdb.DatabaseAccountArgs{
			...
			Kind:                     documentdb.DatabaseAccountKindMongoDB,
		...
		// Create user-assigned identity for the cosmosdb account
		userAssignedIdentity, err := managedidentity.NewUserAssignedIdentity(ctx, "userAssignedIdentity", &managedidentity.UserAssignedIdentityArgs{
			ResourceGroupName: resourceGroup.Name,
			ResourceName:      pulumi.Sprintf("mongo-%v-id", randomName),
		})
		if err != nil {
			return err
		}

		// // Create role-assignment for the user-assigned identity to access the cosmosdb account
		// _, err = authorization.NewRoleAssignment(ctx, "mongoRoleAssignment", &authorization.RoleAssignmentArgs{
		// 	PrincipalId:      userAssignedIdentity.PrincipalId,
		// 	RoleDefinitionId: pulumi.String("/providers/Microsoft.Authorization/roleDefinitions/5bd9cd88-fe45-4216-938b-f97437e15450"),
		...
		azureMonitorWorkspace, err := monitor.NewAzureMonitorWorkspace(ctx, "azureMonitorWorkspace", &monitor.AzureMonitorWorkspaceArgs{
			ResourceGroupName:         resourceGroup.Name,
			AzureMonitorWorkspaceName: pulumi.Sprintf("prometheus-%v", randomName),
		})
		if err != nil {
			return err
		}
		...
		grafanaDashboard, err := dashboard.NewGrafana(ctx, "grafanaDashboard", &dashboard.GrafanaArgs{
			ResourceGroupName: resourceGroup.Name,
			WorkspaceName:     pulumi.Sprintf("grafana-%v", randomName),
		...
		ctx.Export("databaseAccountId", databaseAccount.ID())

		ctx.Export("databaseAccountIdentityId", userAssignedIdentity.ID())

		ctx.Export("databaseAccountIdentityClientId", pulumi.All(userAssignedIdentity.ClientId).ApplyT(
			func(args []interface{}) string {
				return args[0].(string)
			}))

		ctx.Export("databaseAccountListConnectionStringUrl",
			pulumi.Sprintf("https://management.azure.com/subscriptions/%v/resourceGroups/%v/providers/Microsoft.DocumentDB/databaseAccounts/%v/listConnectionStrings?api-version=2021-04-15",
				client.SubscriptionId, resourceGroup.Name, databaseAccount.Name))

		ctx.Export("databaseAccountScope", pulumi.String("https://management.azure.com/.default"))
