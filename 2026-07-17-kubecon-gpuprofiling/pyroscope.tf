...
resource "helm_release" "pyroscope_operator" {
  ...
  set = concat(
    [
      ...
      {
        # Azure Managed Grafana provisions each managed private endpoint's
        # underlying private endpoint in a Microsoft-owned platform subscription
        # (per Grafana instance/region), NOT in our subscription. The CCM matches
        # azure-pls-auto-approval against the PE's subscription, so this must be
        # that AMG platform subscription for the pending connection to auto-approve.
        # Stable for this Grafana instance but undocumented; update if you rebuild
        # Grafana in a different region. See the README ("Look up the auto-approval
        # subscription ID") for the full az lookup from the AKS name + resource group.
        name  = "pyroscope.service.annotations.service\\.beta\\.kubernetes\\.io/azure-pls-auto-approval"
        value = "e08067a5-cca7-453a-87df-eb2ddd9d60d6"
        type  = "string"
      },
      {
        name  = "alloy.enabled"
...
data "azurerm_private_link_service" "example" {
  resource_group_name = azapi_resource.aks.output.properties.nodeResourceGroup
  name                = "pyroscope-pls"
...
resource "time_sleep" "wait_for_pls_auto_approval" {
  create_duration = "180s"
...
resource "azapi_resource_action" "grafana_managed_private_endpoint_refresh" {
  ...
  depends_on = [
    data.azapi_resource.pyroscope_private_link_service,
    time_sleep.wait_for_pls_auto_approval
  ]
}
